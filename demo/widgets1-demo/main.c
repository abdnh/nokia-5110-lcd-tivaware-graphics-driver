...
tRadioButtonWidget buttons[BUTTONS_NUM] = {
    ColorRadioButton(
        10, 0, 50, 10,
        RB_STYLE_TEXT, ~0, 0, 0,
        "Red", onRedChange)
...
void on_switch(void) {
    ...
    if(!pin4) {
        ...
        selected = selected > 0 ? selected - 1 : BUTTONS_NUM - 1;
    }
    tRadioButtonWidget *btn = &buttons[selected];
    WidgetMessageQueueAdd(&btn->sBase, WIDGET_MSG_PTR_UP,
    btn->sBase.sPosition.i16XMin, btn->sBase.sPosition.i16YMin, false, false);
